...
void BSpline::geraCurva(ListaEnc<Coordenada>& controle) {
	...
	for (int i = 0; i < controle.tamanho() - 3; i++) {
		...
		// Cx = Mbs * Gx  // Cy = Mbs * Gy
		cx[0] = 1/6 * (-1*p1.getX() + 3*p2.getX() - 3*p3.getX() + 1*p4.getX());
		cx[1] = 1/6 * (3*p1.getX() - 6*p2.getX() + 3*p3.getX());
		cx[2] = 1/6 * (-3*p1.getX() + 3*p3.getX());
		cx[3] = 1/6 * (p1.getX() + 4*p2.getX() + p3.getX());

		cy[0] = 1/6 * (-1*p1.getY() + 3*p2.getY() - 3*p3.getY() + 1*p4.getY());
		cy[1] = 1/6 * (3*p1.getY() - 6*p2.getY() + 3*p3.getY());
		cy[2] = 1/6 * (-3*p1.getY() + 3*p3.getY());
		cy[3] = 1/6 * (p1.getY() + 4*p2.getY() + p3.getY());
